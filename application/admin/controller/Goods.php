...
class Goods extends Base {
    ...
    //ecs数据库
    //$test1=Db::connect("DB_Config_1")->name("ecs_users")->select();
    ...
    public function ajaxGoodsList(){            
        ...
        (I('db_is_on_sale') !== '') && $where = "$where and db_is_on_sale = ".I('db_is_on_sale') ; 
        $cat_id = I('cat_id');
        // 关键词搜索               
        ...
        $count = Db::connect("DB_Config_1")->name("ecs_goods")->where($where)->count();
        $Page  = new AjaxPage($count,20);
        /**  搜索条件下 分页赋值
        ...
        */
        ...
        $goodsList = Db::connect("DB_Config_1")->name("ecs_goods")->where($where)->limit($Page->firstRow.','.$Page->listRows)->select();

        foreach($goodsList as $key=>$goods){
            $goodsList[$key]['cat_name'] = Db::connect("DB_Config_1")->name("ecs_category")->where('cat_id', $goods['cat_id'])->value('cat_name');
        }
    ...
    public function addEditGoods()
    {
        ...
        if($goods_id){
            ...
            $goods = Db::connect("DB_Config_1")->name("ecs_goods")->where('goods_id', $goods_id)->Field('*,goods_weight as weight,goods_number as store_count,virtual_sales as virtual_sales_sum,goods_desc as goods_content,goods_volume as volume,goods_brief as goods_remark')->find();
            ...
            //商品相册
            $goods_images = Db::connect("DB_Config_1")->name("ecs_goods_gallery")->where('goods_id', $goods['goods_id'])->Field('*,img_url as image_url')->select();
        ...
        $cat_list = Db::connect("DB_Config_1")->name("ecs_category")->where("parent_id = 0")->Field('cat_id as id,cat_name as name,parent_id')->select();

        //$goodsType = Db::name("GoodsType")->select();
        $goodsType = Db::connect("DB_Config_1")->name("ecs_attribute")->Field('*,attr_name as name')->select();
    ...
    public function ajaxGetSpecSelect()
    {
        ...
        foreach($specList as $key =>$spec){
            ...
            $specList[$key]['product_number'] = $spec['product_number'];
            
            $specList[$key]['db_price'] = $goods_attr['db_price'];
            $specList[$key]['shop_price'] = $goods_attr['attr_price'] + Db::connect("DB_Config_1")->name("ecs_goods")->where("goods_id", $spec['goods_id'])->value('shop_price');
        }
