...
class GoodsLogic extends Model
{
    ...
    public function getSpecInput($goods_id, $spec_arr)
    {
       ...
       foreach ($spec_arr2 as $k => $v) 
       {
            ...
            $str .="<td><input name='item[$item_key][product_sn]' value='{$keySpecGoodsPrice[$item_key][product_sn]}' onkeyup='this.value=this.value.replace(/[^\d.]/g,\"\")' onpaste='this.value=this.value.replace(/[^\d.]/g,\"\")' /></td>";
    ...
    function find_parent_cat($cat_id)
    {
        ...
        $cat_list = Db::connect("DB_Config_1")->name("ecs_category")->Field('cat_id as id,cat_name as name,parent_id')->select();
        $cat_level_arr[$cat_list[$cat_id]['level']] = $cat_id;


        $parent_id = Db::connect("DB_Config_1")->name("ecs_category")->where('cat_id',$cat_id)->value('parent_id');
        $cat_level_arr[1] = $parent_id;
        $cat_level_arr[2] = $cat_id;
        // 找出他老爸
    ...
    function getSortBrands($cat_id=0)
    {
        ...
        $brandList = Db::connect("DB_Config_1")->name("ecs_brand")->Field('brand_id as id,brand_name as name')->select();
    ...
    function getSortCategory()
    {
        ...
        $categoryList = Db::connect("DB_Config_1")->name("ecs_category")->Field('cat_id as id,cat_name as name,parent_id')->select();
        $nameList = array();
