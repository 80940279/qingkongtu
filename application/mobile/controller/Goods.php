...
class Goods extends MobileBase
{
    ...
    public function goodsList()
    {
        ...
        if($id){
            ...
            $cate_data = Db::connect("DB_Config_1")->name("ecs_category")->Field('cat_id as id,parent_id')->select();
            foreach($cate_data as $cate){
                $cat_id_arr[] =  $cate['id'];
            }
        }

        $goods_where = ['is_delete'=>0,'db_is_on_sale' => 1, 'cat_id' => ['in', $cat_id_arr]];
        $filter_goods_id = Db::connect("DB_Config_1")->name("ecs_goods")->where($goods_where)->getField("goods_id", true);
        ...
        if ($count > 0) {
            ...
            $goods_list = Db::connect("DB_Config_1")->name("ecs_goods")->where("goods_id", "in", implode(',', $filter_goods_id))->order(['goods_id' => $sort_asc])->limit($page->firstRow . ',' . $page->listRows)->select();
        ...
        $this->assign('exchange_rate', $this->get_exchangerate());
        $this->assign('cateArr', $cateArr);
        $this->assign('filter_param', $filter_param); // 帅选条件
        $this->assign('cat_id', $id);
        $this->assign('is_fcate', 1);
    ...
    public function goodsInfo()
    {
        ...
        $goods = Db::connect("DB_Config_1")->name("ecs_goods")->where("goods_id",$goods_id)->find();
        ...
        $goods['goods_images'] = Db::connect("DB_Config_1")->name("ecs_goods_gallery")->where("goods_id",$goods_id)->field("thumb_url as image_url")->select();
    ...
    public function goodsinfolist()
    {
        ...
        if($goods['goods_type']){
            $specList = Db::connect("DB_Config_1")->name("ecs_products")->where("goods_id", $goods_id)->select();

            if($specList){
                foreach($specList as $key =>$spec){
                    $goods_attr = Db::connect("DB_Config_1")->name("ecs_goods_attr")->where("goods_attr_id", $spec['goods_attr'])->find();
                    $specList[$key]['item_id'] = $goods_attr['goods_attr_id'];
                    $specList[$key]['item'] = $goods_attr['attr_value'];

                    $spec_goods_price[$goods_attr['goods_attr_id']]['key'] = $goods_attr['goods_attr_id'];
                    $spec_goods_price[$goods_attr['goods_attr_id']]['item_id'] = $spec['product_id'];
                    $spec_goods_price[$goods_attr['goods_attr_id']]['store_count'] = $spec['product_number']; 
                    $spec_goods_price[$goods_attr['goods_attr_id']]['price'] = $goods_attr['db_price'];
                    
                    //$spec_goods_price[$key]['price'] = $goods_attr['attr_price'] + Db::connect("DB_Config_1")->name("ecs_goods")->where("goods_id", $spec['goods_id'])->value('shop_price');
                }
                $filter_spec[Db::connect("DB_Config_1")->name("ecs_attribute")->where("attr_id", $goods_attr['attr_id'])->getField("attr_name")] = $specList;
            }
        }
         ...
         * 分类商品列表，搜索出的商品列表，点击购物车按钮弹出
        */
        public function goodsinfolists()
        {
            C('TOKEN_ON',true);        
            $goodsLogic = new GoodsLogic();
            $goods_id = I("post.id/d");
    ...
    public function search()
    {
        ...
        if ($count > 0) {
            ...
            $sort_arr = ['virtual_sales','shop_price','is_new','sort_order'];
            if(!in_array($sort,$sort_arr)) $sort='sort_order';
            $goods_list = Db::connect("DB_Config_1")->name("ecs_goods")->where("goods_id", "in", implode(',', $filter_goods_id))->order([$sort => $sort_asc])->limit($page->firstRow . ',' . $page->listRows)
                ->field('goods_id,goods_name,shop_price,virtual_sales')->select();
