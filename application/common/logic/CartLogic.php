...
class CartLogic extends Model
{
    ...
    public function setGoodsModel($goods_id)
    {
        if ($goods_id > 0) {
            ...
            $this->goods = Db::connect("DB_Config_1")->name("ecs_goods")->where(['goods_id'=>$goods_id,'db_is_on_sale'=>1])->find();
        }
    }
    ...
    public function buyNow()
    {
        ...
        if (empty($this->specGoodsPrice)) {
            ...
            $specGoodsPriceCount =  Db::connect("DB_Config_1")->name("ecs_products")->where("goods_id", $goods_id)->count('product_id');
    ...
    private function addNormalCart()
    {
        if (empty($this->specGoodsPrice)) {
            ...
            $store_count = $this->goods['goods_number'];
        } else {
...
//        halt(($promGoodsLogic->checkActivityIsEnd() || !$promGoodsLogic->checkActivityIsAble()));
    ...
    public function getCartList($selected = 0)
    {
        ...
        foreach($cartList as $k=>$cart){
            $cartList[$k]['goods'] = Db::connect("DB_Config_1")->name("ecs_goods")->where("goods_id",$cart['goods_id'])->find();
        }
    ...
    public function checkCartList($cartList)
    {
        ...
        foreach ($cartList as $cartKey => $cart) {
            //商品不存在或者已经下架
            if (empty($cart['goods']) || $cart['goods']['db_is_on_sale'] != 1 || $cart['goods_num'] == 0) {
                $cart->delete();
