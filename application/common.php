...
function getCatGrandson ($cat_id)
{
    ...
    $GLOBALS['category_id_arr'] = Db::connect("DB_Config_1")->name("ecs_category")->getField('cat_id as id,parent_id');
    // 先把所有儿子找出来
    //$son_id_arr = M('GoodsCategory')->where("parent_id", $cat_id)->cache(true,TPSHOP_CACHE_TIME)->getField('id',true);
    $son_id_arr = Db::connect("DB_Config_1")->name("ecs_category")->where("parent_id", $cat_id)->getField('cat_id as id',true);
    print_r($son_id_arr);die;
...
function get_goods_category_tree(){
    ...
    $cat_list = Db::connect("DB_Config_1")->name("ecs_category")->where(['is_show' => 1])->order('cat_id')->select();
    if($cat_list){
        foreach ($cat_list as $val){ 
            if($val['parent_id'] == 0){
                $tree[] = $val;
