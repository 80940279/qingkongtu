...
function changeTableVal(table, id_name, id_value, field, obj,yes,no) {
	...
	if ($(obj).hasClass('noexa')) // 图片点击是否操作
	{
		//src = '/public/images/yes.png';
		$(obj).removeClass('noexa').addClass('yesexa');
		$(obj).html("<i class='fa fa-check-circle'></i>通过");
		value = 1;
	} else if ($(obj).hasClass('yesexa')) { // 图片点击是否操作
		$(obj).removeClass('yesexa').addClass('noexa');
		$(obj).html("<i class='fa fa-ban'></i>未通过");
		value = 0;
	}
