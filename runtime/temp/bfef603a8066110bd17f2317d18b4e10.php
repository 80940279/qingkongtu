<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:52:"./application/admin/view/goods\ajax_spec_select.html";i:1575368542;}*/ ?>
<style>
    input[type=checkbox] {
        border: 0;
    ...
    .check_noselected {
        width: 12px;
        height: 12px;
        background: url("/public/static/images/no-selected.png") no-repeat;
    ...
    .check_selected {
        width: 12px;
        height: 12px;
        background: url("/public/static/images/selected.png") no-repeat;
        background-size: cover;
        vertical-align: middle;
    }
</style>
<table class="table table-bordered" id="goods_spec_table1">
    ...
    <!--<tr>
        ...
        <td>                            
            <?php if(is_array($vo[spec_item]) || $vo[spec_item] instanceof \think\Collection || $vo[spec_item] instanceof \think\Paginator): if( count($vo[spec_item])==0 ) : echo "" ;else: foreach($vo[spec_item] as $k2=>$vo2): ?>                            
                ...
                <input type="checkbox" data-spec_id='<?php echo $vo[id]; ?>' data-item_id='<?php echo $k2; ?>' class="<?php
                             if(in_array($k2,$items_ids))
                                    echo 'check_selected';
                             else
                                echo 'check_noselected';
                             ?>">
                <?php echo $vo2; ?>
                &nbsp;&nbsp;
    ...
    <tr>
        <td><?php echo $vo['product_sn']; ?></td>
        <td><img width="50" src="http://www.chunsun.com/data/product/<?php echo $vo['product_thumb']; ?>" alt="img"></td>
        <td><?php echo $vo['shop_price']; ?></td> 
        <td><input style="width: 70px;" type="text" onKeyUp="this.value=this.value.replace(/[^\d]/g,'')" onpaste="this.value=this.value.replace(/[^\d]/g,'')" onblur="changeTableVal('ecs_goods_attr','goods_attr_id','<?php echo $vo['goods_attr']; ?>','db_price',this)" size="4" value="<?php echo $vo['db_price']; ?>" /></td>
        <td><?php echo $vo['product_weight']*1000; ?></td>
        <td><?php echo $vo['product_number']; ?></td>
    </tr>                             
    <?php endforeach; endif; else: echo "" ;endif; ?>    
</table>
...
<script>
   ...
   // 规格按钮切换 class
   $("#ajax_spec_data :checkbox").click(function(){
	   if($(this).hasClass('check_selected'))
	   {
		   $(this).removeClass('check_selected');
		   $(this).addClass('check_noselected');
	   }else{
		   $(this).removeClass('check_noselected');
...
//	  spec_arr[2] = [3,4]; 
	...
	// 选中了哪些属性	  
	$("#goods_spec_table1 :checkbox").each(function(){
	    if($(this).hasClass('check_selected'))
		{
        ...
        success: function (data) {
            ...
            $("#spec_input_tab").find('tr').each(function (index, item) {
                ...
                if(store_count == 0 && price == 0){
                    console.log('ajaxGetSpecInput2')
                    //$(this).find(".delete_item").trigger('click');
                }
				//供应商商品定死库存和成本价
				if (goods_behavior == 'audit' || goods_behavior == 'editSupplierGoods') {
					var tr = $('#goods_spec_table2').find('table tbody tr');
					tr.find('td:eq(2) input').attr('readonly','readonly');
					tr.find('td:eq(4) input').attr('readonly','readonly');
				}
            });
        }
    });
