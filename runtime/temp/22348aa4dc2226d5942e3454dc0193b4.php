<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:56:"./application/admin/view/distribut\ajaxsecond_users.html";i:1575706358;}*/ ?>
    ...
    <table>
        ...
        <?php if(is_array($userList) || $userList instanceof \think\Collection || $userList instanceof \think\Paginator): $i = 0; $__LIST__ = $userList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?>
            <tr data-id="<?php echo $list['user_id']; ?>">
                ...
                <td align="left" class="">
                    <div style="text-align: center; width: 100px;"><?php echo (isset($first_leader[$list[user_id]]['count']) && ($first_leader[$list[user_id]]['count'] !== '')?$first_leader[$list[user_id]]['count']:"0"); ?></div>
                </td>
                <!--<td align="left" class="">
                ...
                <td align="center" axis="col0">
                    <div style="text-align: center; width: 80px;">
                        <?php if($list[is_examine] == 1): ?>
                        <span class="yesexa" onClick="changeTableVal('users','user_id','<?php echo $list['user_id']; ?>','is_examine',this)" ><i class="fa fa-check-circle"></i>通过</span>
                        <?php else: ?>
                        <span class="noexa" onClick="changeTableVal('users','user_id','<?php echo $list['user_id']; ?>','is_examine',this)" ><i class="fa fa-ban"></i>未通过</span>
                        <?php endif; ?>
                    </div>
                </td> 
                <td align="left" class="">
                    <div style="text-align: center; width: 150px;"><?php echo date('Y-m-d H:i',$list['reg_time']); ?></div>
                </td>
                <td align="center" class="handle">
                    <div style="text-align: center; width: 170px; max-width:250px;">
                        <a class="btn blue" href="<?php echo U('Admin/distribut/second_detail',array('id'=>$list['user_id'])); ?>">修改</a>
                        <a class="btn blue" href="<?php echo U('Admin/user/account_log',array('id'=>$list['user_id'])); ?>">订单详情</a>
